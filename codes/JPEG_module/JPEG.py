...
class JPEG(nn.Module):
    def __init__(self,quality_factor,compress,quantize=None):
        ...
        factor = 5000 / self.quality_factor if self.quality_factor < 50 else 200 - 2 * self.quality_factor
        self.device = 'cuda'
    ...
    def forward(self, input):
        if self.compress: #Input is an image:
            ...
            output = output/self.Q_table_luminance
            if self.quantize:
                output = torch.round(output)
            output = output.contiguous().view([input.size(0),64,input.size(2)//8,input.size(3)//8])
        else:# Input is DCT blocks
            output = input.view([input.size(0),8,8,input.size(2),input.size(3)])*self.Q_table_luminance
