...
from .util import post_process_k,analytic_kernel
...
def train(conf):
    ...
    if conf.X4:
        final_kernel = analytic_kernel(final_kernel)
    return final_kernel
    # gan.finish()
...
def main():
    ...
    for filename in [file for file in os.listdir(os.path.abspath(args.input_dir)) if os.path.isfile(os.path.join(os.path.abspath(args.input_dir),file))]:
