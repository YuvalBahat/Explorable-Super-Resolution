...
class DecompCNNModel(BaseModel):
    def __init__(self, opt,accumulation_steps_per_batch=None,init_Fnet=None,init_Dnet=None,chroma_mode=False,**kwargs):
        ...
        logs_2_keep = ['l_g_pix_log_rel', 'l_g_fea', 'l_g_range', 'l_g_gan', 'l_d_real', 'l_d_fake','D_loss_STD','l_d_real_fake',
                       ...
                       'per_pix_STD_val']+['l_g_latent_%d'%(i) for i in range(self.num_latent_channels)]
        ...
        if self.is_train:
            ...
            assert self.opt['network_D']['input_type'] in ['DCT_postmult','DCT_premult']
            self.post_mult_discriminator = self.opt['network_D']['input_type']=='DCT_postmult'
            ...
            if self.D_exists:
                ...
                self.netD.train()
                self.skipped_G_steps = None
            self.netG.train()
            self.mixed_Y_4_training = self.D_exists and self.chroma_mode
        else:
            self.mixed_Y_4_training,self.D_exists,self.post_mult_discriminator = False,False,False
        ...
        if self.is_train:
            ...
            elif train_opt['lr_scheme'] is not None:
    ...
    def optimize_parameters(self):
        ...
        for possible_dual_step_num in range(actual_dual_step_steps):
            ...
            if optimized_Z_step:
                ...
                self.fake_H = self.netG(self.model_input)
            self.output_image = self.JPEG['extractor'](self.fake_H)
            if self.chroma_mode:
                    ...
                    if first_grad_accumulation_step_D and first_dual_batch_step:
                        ...
                        if self.generator_step:
                            ...
                            if not self.generator_step:
                                ...
                                if self.skipped_G_steps is None:
                                    self.skipped_G_steps = self.gradient_step_num
                            else:
                                if self.skipped_G_steps is not None:
                                    print('Skipped G steps %d-%d, since D was not verified'%(self.skipped_G_steps,self.gradient_step_num))
                                    self.skipped_G_steps = None
                    if self.discriminator_step:
            ...
            if self.generator_step or self.generator_Z_opt_only_step:
                ...
                if self.generator_step:
                    if not optimized_Z_step:
                        if self.cri_pix:
                            quantized_l_pix = self.cri_pix(self.crop_2_output_shape(self.JPEG['extractor'](self.var_Comp),DCT=False), self.crop_2_output_shape(self.var_Uncomp,DCT=False))
    ...
    def test_Y_(self,detach=False):
        ...
        self.y_channel_input = torch.clamp(self.JPEG['extractor_Y'](self.y_channel_input),0,255)
        if detach:
        ...
        for val_data in tqdm.tqdm(data_loader):
            ...
            if collect_avg_err_est:
                if self.chroma_mode:
                    ...
                    DCT_diffs = (self.var_Comp-self.fake_H).abs()
                if self.G_margins != 0:
                ...
                if first_eval:
                    GT_DCT_diffs = (self.var_Comp-self.JPEG['non_quantized_compressor'](self.var_Uncomp)).abs()
                    if self.chroma_mode:
            ...
            if save_images:
                if SAVE_IMAGE_COLLAGE:
                    ...
                    if first_eval:  # Save GT Uncomp images
                        ...
                        quantized_image = util.tensor2img(self.JPEG['extractor'](self.var_Comp),out_type=np.uint8, min_max=[0, 255],chroma_mode=chroma_mode)
