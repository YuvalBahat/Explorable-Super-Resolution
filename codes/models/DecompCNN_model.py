...
class DecompCNNModel(BaseModel):
    def __init__(self, opt,accumulation_steps_per_batch=None,init_Fnet=None,init_Dnet=None,**kwargs):
        ...
        self.step = 0
        self.jpeg_compressor = JPEG(quality_factor=50, compress=True, quantize=True).to(self.device)
        self.jpeg_extractor = JPEG(quality_factor=50, compress=False).to(self.device)
    ...
    def feed_data(self, data, need_Uncomp=True):
        ...
        if self.latent_input is not None:
            ...
            if isinstance(cur_Z,int) or len(cur_Z.shape)<4 or (cur_Z.shape[2]==1 and not torch.is_tensor(cur_Z)):
                cur_Z = cur_Z*np.ones([1,self.num_latent_channels]+[self.Z_size_factor*val for val in list(self.var_Comp.size()[2:])])
            elif torch.is_tensor(cur_Z) and cur_Z.size(dim=2)==1:
                cur_Z = (cur_Z*torch.ones([1,1]+[self.Z_size_factor*val for val in list(self.var_Comp.size()[2:])])).type(self.var_Comp.type())
        ...
        for possible_dual_step_num in range(actual_dual_step_steps):
            ...
            if first_dual_batch_step:
                if self.num_latent_channels>0:
                    ...
                    static_Z = None
            if optimized_Z_step:
                self.Z_optimizer.feed_data({'Comp':self.var_Comp,'Uncomp':self.var_Uncomp})
            ...
            if not self.D_exists:
                ...
                if ((self.gradient_step_num) % max([1,np.ceil(1/self.cur_D_update_ratio)]) == 0) and self.gradient_step_num > -self.D_init_iters:
                    ...
                    if first_dual_batch_step:
                        pred_d_real = self.netD(elf.var_ref)
    ...
    def get_current_visuals(self, need_Uncomp=True,entire_batch=False):
        ...
        if entire_batch:
            ...
            out_dict['Decomp'] = (self.fake_H[0]+self.fake_H[1] if isinstance(self.fake_H,list) else self.fake_H).detach()[0].float().cpu()
            if need_Uncomp:
